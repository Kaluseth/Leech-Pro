...
async def zee5_capture(bot, update):
    ...
    if "zee5" in update.text:
        try:
            ...
            await update.reply_text("There's some issue with your URL 馃槙", quote=True)
            return
            
    else:
        await update.reply_text("I can download from Zee5 links only! Use @disneyteamurluploaderbot  for other links 馃槆", quote=True)
        return
    ...
    try:
        ...
        if t_response:
                    ...
                    if format_string is not None and not "audio only" in format_string:
                        ikeyboard = [
                            InlineKeyboardButton(
                                "馃帪 (" + format_string + ") " + approx_file_size + " ",
                                callback_data=(cb_string_video).encode("UTF-8")
                            ),
                            InlineKeyboardButton(
                                "馃搧 FILE " + format_ext + " " + approx_file_size + " ",
            ...
            await update.reply_text("There's some issue with your URL 馃槙 Or may be DRM protected!", quote=True)
            return
    except:
